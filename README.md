...
- 想给玩家提供全球市场、拍卖行和交易 GUI 的服务器。
- 想用服务器商店和回收站调节经济流通的服务器。
- 想在 Velocity 群组服中同步市场、拍卖、邮件和价格压力的服务器。
- 想让物品价格随库存、回收量和全服经济情况变化的服务器。
...
个人商店会只显示某个玩家正在出售的商品，适合做玩家摊位、城镇商铺或牌子入口。
...
服务器商店由服主配置，玩家直接向服务器购买物品。商店支持三类商品：

- 固定无限库存：适合常驻基础商品。
- 固定有限库存：适合限量供应商品。
- 回流库存商品：库存来自玩家回收站卖出的物品。
...
玩家可以把指定物品卖给服务器。回收价格会受配置、回收压力、周期波动和全服经济阶段影响。
...
`v1.1.1` 增加了新的动态价格能力。服主可以继续使用固定价格，也可以让价格根据服务器经济状态变化。
...
- 回流库存商品会根据回流库存多少调整售价。
- 回流商品有防套利保护，避免玩家低买高卖刷钱。

### 跨服价格压力同步

Velocity 群组服中，一个子服的回收压力可以同步到其他子服。玩家在 A 服大量回收后，B 服的相关价格也会跟着变化。

跨服模式必须同时安装同版本的 Paper 主插件和 Velocity 插件。
...
- Paper 1.21 或更新版本
- Java 版本按你的 Paper 服务端要求准备
- Vault
- 一个支持 Vault 的经济插件

### 下载文件

从 GitHub Release 下载：<https://github.com/YoruSakura/GensouMarket/releases/tag/v1.2>

- `GensouMarket-plugin-1.2.jar`：放到 Paper 子服的 `plugins` 文件夹。
- `GensouMarket-velocity-1.2.jar`：跨服模式才需要，放到 Velocity 的 `plugins` 文件夹。
- `GensouMarket-1.2-sha256.txt`：用于校验下载文件。

### 单服安装

1. 把 `GensouMarket-plugin-1.2.jar` 放进 Paper 服务器的 `plugins` 文件夹。
...
1. 每个 Paper 子服都安装 `GensouMarket-plugin-1.2.jar`。
2. Velocity 代理端安装 `GensouMarket-velocity-1.2.jar`。
...
跨服模式必须使用 MySQL。升级或替换插件时，Paper 主插件和 Velocity 插件请保持同版本。
...
- 拍卖取消后，手续费不会退还。
- 跨服环境中遇到同步问题时，先确认 Paper 和 Velocity 两个插件都是同一版本。
