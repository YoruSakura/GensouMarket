...
- 真正的经济扣款、发物品、打开界面、发聊天提示，都必须由 Paper 端执行
...
- 记录本服当前哪些玩家正在查看市场界面 / 拍卖列表 / 拍卖详情
- 记录详情页对应的 `auctionId`
- 在收到跨服刷新事件时，找到应该刷新的玩家并刷新

这一步很重要，因为当前界面刷新逻辑只会刷新本服当前操作玩家自己的界面，不会自动跨服通知。
...
8. 广播 `MARKET_LISTING_SOLD` 给所有后端，用于刷新界面 / 提示
...
- 目标服自己重新从数据库读取最新状态后刷新界面

这和现有界面逻辑更一致，也能避免消息体过大。
